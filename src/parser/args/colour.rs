use std::{collections::HashMap, fmt::{self, Display, Write}, num::ParseIntError, str::FromStr, sync::LazyLock};
use thiserror::Error;
...
impl FromStr for Rgba {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if s.starts_with("%[") && s.ends_with("]") {
            let colour_name = &s[1 .. s.len()-1];

            if let Some(colour) = COLOURS.get(colour_name) {
                Ok(*colour)
            }
            else {
        ...
        else { Err(ColourParseError::InvalidColourLiteral(s.to_string())) }
    }
}
...
impl Rgba {
    ...
    pub(super) fn from_named_literal_unchecked(literal: &str) -> Option<Self>{
        let colour_name = &literal[1 .. literal.len()-1];
        COLOURS.get(colour_name).copied()
    }
    ...
    pub(super) fn from_hex_literal_unchecked(literal: &str) -> Self {
        let mut channels = (1..literal.len()).step_by(2).map(|start| literal.get(start..start + 2));
            let mut get_channel = || {
